...
const getThematicIllustration = (text) => {
  ...
  if (lowerText.includes('кофе') || lowerText.includes('чай') || lowerText.includes('паузу') || lowerText.includes('отдых') || lowerText.includes('ничегонеделания') || lowerText.includes('напитка') || lowerText.includes('уют'))
    return <Coffee {...props} color="#795548" />;
  if (lowerText.includes('солнце') || lowerText.includes('свет') || lowerText.includes('тепла') || lowerText.includes('луч') || lowerText.includes('огоньки') || lowerText.includes('лампочку'))
    return <Sun {...props} color="#F59E0B" />;
  if (lowerText.includes('сердце') || lowerText.includes('любовь') || lowerText.includes('обними') || lowerText.includes('себя') || lowerText.includes('любишь') || lowerText.includes('словами'))
    return <Heart {...props} color="#c82926" />;
  if (lowerText.includes('снежинк') || lowerText.includes('небо') || lowerText.includes('тишин') || lowerText.includes('атмосферу') || lowerText.includes('снег') || lowerText.includes('декабрь'))
    return <CloudSun {...props} color="#706c91" />;
  if (lowerText.includes('творчеств') || lowerText.includes('вдохнов') || lowerText.includes('мысли') || lowerText.includes('хаос') || lowerText.includes('мечтать'))
    return <Feather {...props} color="#c82926" />;
  if (lowerText.includes('ошибк') || lowerText.includes('пазл') || lowerText.includes('решение') || lowerText.includes('очищение'))
    return <Puzzle {...props} color="#706c91" />;
  if (lowerText.includes('кот') || lowerText.includes('животное')) return <Cat {...props} color="#363636" />;
  if (lowerText.includes('нет') || lowerText.includes('границы') || lowerText.includes('защит') || lowerText.includes('опоры'))
  ...
  if (lowerText.includes('энерги') || lowerText.includes('заряд') || lowerText.includes('сил'))
    return <BatteryCharging {...props} color="#706c91" />;
  if (lowerText.includes('улыб') || lowerText.includes('смех') || lowerText.includes('радость') || lowerText.includes('хорошо') || lowerText.includes('счастливым') || lowerText.includes('удовольствие'))
    return <Smile {...props} color="#F59E0B" />;
  if (lowerText.includes('дыши') || lowerText.includes('ветер') || lowerText.includes('воздух') || lowerText.includes('выдохни'))
    return <Wind {...props} color="#706c91" />;
  if (lowerText.includes('время') || lowerText.includes('минут') || lowerText.includes('часы') || lowerText.includes('темп') || lowerText.includes('замедлиться'))
    return <Clock {...props} color="#363636" />;
  if (lowerText.includes('книг') || lowerText.includes('чита') || lowerText.includes('автор') || lowerText.includes('глава') || lowerText.includes('истори'))
    return <BookOpen {...props} color="#795548" />;
  if (lowerText.includes('шаг') || lowerText.includes('вперед') || lowerText.includes('старт') || lowerText.includes('действи') || lowerText.includes('начала'))
    return <Rocket {...props} color="#c82926" />;
  if (lowerText.includes('компас') || lowerText.includes('путь') || lowerText.includes('направлени') || lowerText.includes('контроль') || lowerText.includes('подсказку'))
    return <Compass {...props} color="#c82926" />;
  if (lowerText.includes('чудо') || lowerText.includes('маги') || lowerText.includes('искренне') || lowerText.includes('огоньком') || lowerText.includes('волшебство'))
    return <Sparkles {...props} color="#F59E0B" />;
  if (lowerText.includes('важен') || lowerText.includes('король') || lowerText.includes('достижение') || lowerText.includes('похвал') || lowerText.includes('лидерство') || lowerText.includes('побед') || lowerText.includes('гордишься'))
    return <Crown {...props} color="#F59E0B" />;
  if (lowerText.includes('праздник') || lowerText.includes('вечерин') || lowerText.includes('ритуал') || lowerText.includes('ёлку') || lowerText.includes('новый год'))
    return <PartyPopper {...props} color="#c82926" />;
  if (lowerText.includes('друг') || lowerText.includes('коллег') || lowerText.includes('люд') || lowerText.includes('знакомств'))
    return <Users {...props} color="#706c91" />;
  if (lowerText.includes('музык') || lowerText.includes('песн') || lowerText.includes('слушай'))
...
export default function AdventCalendar() {
  ...
  // Текущий день декабря (1–31)
  const today = new Date();
  const currentDate = Math.min(Math.max(today.getDate(), 1), 31);
  ...
  const getModalTitle = (day) => `День ${day}`;
        ...
        <div className="grid grid-cols-5 gap-3 sm:gap-4">
          {daysGrid.map((day) => {
            ...
            return (
              ...
              >
                <span
                  className={`font-semibold z-10 ${isFuture ? 'opacity-40' : ''} text-lg`}
                ...
                <div className="mt-1">
                  {isFuture && <Lock size={14} color={COLORS.rodeoDust} className="opacity-60" />}
                  {isOpened && (
      ...
      {modalData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-sm rounded-[24px] p-8 relative shadow-2xl transform transition-all scale-100 flex flex-col items-center text-center animate-in fade-in zoom-in duration-300">
            ...
            <p className="text-lg leading-relaxed mb-8 text-[#363636]">«{modalData.text}»</p>
